...
<template>
  <FormDialog ref="formDialog" v-model="model" title="Crear configuración de sensor" :state="state" :schema="sensorsConfigurationFormSchema" @submit="onSubmit">
    ...
    <template #actions>
      <UButton label="Cancelar" icon="i-heroicons-x-circle" variant="outline" :loading="createSensorConfiguration.status.value === 'pending'" @click="model = false" />
      <UButton label="Crear" type="submit" icon="i-heroicons-plus" :loading="createSensorConfiguration.status.value === 'pending'" />
    </template>
  </FormDialog>
</template>
